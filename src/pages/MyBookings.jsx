...
function MyBookings() {
  ...
  return (
    <div className="min-h-screen pt-20 pb-10 bg-premium-bg">

      {/* HEADER */}
      <div className="border-b px-6 py-8 border-premium-border">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-black text-primary-text uppercase tracking-wider">My Bookings</h1>
              <p className="text-gray-600 text-sm mt-1">Your turf booking history</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              { label: "Active", value: bookedCount, color: "#16A34A" },
              { label: "Total Spent", value: `₹${totalSpent.toLocaleString("en-IN")}`, color: "#22C55E" },
            ].map(({ label, value, color }) => (
              <div key={label} className="rounded-xl p-4 border border-premium-border bg-white">
      ...
      <div className="max-w-4xl mx-auto px-6 mt-6">
        <div className="flex gap-2 mb-6">
          {[
            ...
            <button key={key} onClick={() => setFilter(key)}
              ...
              style={{
                backgroundColor: filter === key ? "#22C55E" : "white",
        ...
        {!loading && filtered.length > 0 && (
          <div className="flex flex-col gap-3">
            {filtered.map((b) => (
              <div key={b._id}
                className="rounded-xl border p-5 transition-all duration-200 bg-white"
                style={{
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  ...
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1.5">
                        ...
                        {/* Duration */}
                        <span className="text-gray-600 text-xs">⏱ {calcDuration(b.startTime, b.endTime)} hr</span>
                        {/* Players */}
                        {b.players > 0 && (
                      ...
                      {b.status === "cancelled" && b.refundStatus === "initiated" && (
                        <div className="mt-2.5 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-accent-light text-emerald-900">
                          💸 Refund Initiated — 5-7 business days
                        </div>
                      )}
                      {b.status === "cancelled" && b.refundStatus === "failed" && (
                        <div className="mt-2.5 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-red-50 text-red-600">
                          ⚠️ Refund Failed — Contact support
                        </div>
                      )}
                      {b.status === "cancelled" && b.refundStatus === "not_applicable" && (
                        <div className="mt-2.5 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-600">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0 ml-7 sm:ml-0">
                    {/* Amount */}
                    <span className="text-premium-accent font-black text-base">₹{b.totalPrice}</span>

                    {/* Payment badge */}
                    <span className="text-xs font-black px-2.5 py-1 rounded-full uppercase"
                      style={{
                        backgroundColor:
                          b.paymentStatus === "paid" ? "#DCFCE7" :
                          b.paymentStatus === "refunded" ? "#DBEAFE" : "#F3E8FF",
                        color:
                          b.paymentStatus === "paid" ? "#16A34A" :
                          b.paymentStatus === "refunded" ? "#0284C7" : "#7C3AED",
                      }}>
                    ...
                    {b.status === "booked" && (
                      <button onClick={() => handleCancel(b._id)} disabled={cancellingId === b._id}
                        className="text-xs font-bold uppercase px-4 py-1.5 rounded-full border transition-all hover:bg-red-50 hover:border-red-200 disabled:opacity-50 text-red-600"
        ...
        {!loading && bookings.length > 0 && filtered.length === 0 && (
          <div className="text-center py-16 rounded-2xl border border-premium-border bg-premium-secondary">
            <p className="text-4xl mb-3">🔍</p>
            <p className="text-gray-600 font-semibold">No {filter} bookings found</p>
          </div>
        )}

        <button onClick={() => navigate("/profile")}
          className="mt-8 text-gray-600 text-sm hover:text-gray-700 transition">
          ← Back to Profile
        </button>
      </div>
    </div>
